...
OShaderProgram::~OShaderProgram()
{
	for (ui32 i=0; i < 2; i++)
	{
		glDetachShader(m_programId, m_attachShaders[i]);
		glDeleteShader(m_attachShaders[i]);
	}
...
void OShaderProgram::setUnifoemBufferSlot(const char* name, ui32 slot)
{
...
{
	glLinkProgram(m_programId); i32 logLength = 0;
