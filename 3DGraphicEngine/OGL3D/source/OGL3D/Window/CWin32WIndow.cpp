...
OWindow::OWindow()
{
	...
	m_handle = CreateWindowEx(NULL, MAKEINTATOM(classID), L"3D Graphic Engine", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU,
	...
	// Creating OpenGL Rander Context

	auto hDC = GetDC(HWND(m_handle));


	int piexlFormatAttributes[] = {
	...
	int piexlFormat = 0;
	UINT numFormats = 0;
	wglChoosePixelFormatARB(hDC, piexlFormatAttributes, nullptr, 1, &piexlFormat, &numFormats);
	assert(numFormats);


	PIXELFORMATDESCRIPTOR piexlFormatDesc = {};
	DescribePixelFormat(hDC, piexlFormat, sizeof(PIXELFORMATDESCRIPTOR), &piexlFormatDesc);
	SetPixelFormat(hDC, piexlFormat, &piexlFormatDesc);
